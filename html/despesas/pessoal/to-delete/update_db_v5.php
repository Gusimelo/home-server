...
$sql_check_column = "SHOW COLUMNS FROM `loans` LIKE 'sub_subcategory'";
$result = $conn->query($sql_check_column);

if ($result->num_rows > 0) {
    echo "<h2>A coluna 'sub_subcategory' já existe na tabela 'loans'. Nenhuma ação foi tomada.</h2>";
} else {
    $sql_add_column = "ALTER TABLE `loans` ADD `sub_subcategory` VARCHAR(255) NULL DEFAULT NULL AFTER `subcategory`;";
    if ($conn->query($sql_add_column) === TRUE) {
        echo "<h2 class='success'>Tabela 'loans' atualizada com sucesso! A coluna 'sub_subcategory' foi adicionada.</h2>";
    } else {
        echo "<h2 class='error'>Erro ao atualizar a tabela 'loans': " . $conn->error . "</h2>";
    }
}
